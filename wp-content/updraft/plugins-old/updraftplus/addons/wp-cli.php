...
class UpdraftPlus_CLI_Command extends WP_CLI_Command {
	...
	public function backup($args, $assoc_args) {
		...
		if (isset($assoc_args['incremental']) && filter_var($assoc_args['incremental'], FILTER_VALIDATE_BOOLEAN)) {
			...
			if (empty($nonce)) WP_CLI::error(__('No previous backup found to add an increment to.', 'updraftplus'), $exit = true);
	...
	private function addon_not_exist_error($option, $addon_title, $addon_buy_url) {
		...
		WP_CLI::error(sprintf(__('You have just given the %1$s option. The %1$s is working with "%2$s" addon. Get the "%2$s" addon: %3$s', 'updraftplus'), $option, $addon_title, $filtered_addon_buy_url), true);
	}
