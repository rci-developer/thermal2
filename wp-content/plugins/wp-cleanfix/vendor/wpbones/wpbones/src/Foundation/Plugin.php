...
namespace WPCleanFix\WPBones\Foundation;

use WPCleanFix\WPBones\Container\Container;
use WPCleanFix\WPBones\Database\WordPressOption;
use WPCleanFix\WPBones\View\View;
use WPCleanFix\WPBones\Contracts\Foundation\Plugin as PluginContract;
use WPCleanFix\WPBones\Foundation\Http\Request;
use WPCleanFix\WPBones\Support\Str;
...
class Plugin extends Container implements PluginContract
{
    ...
    public function boot()
    {
         ...
         *
          ...
          Description = "WP CleanFix is a WP Bones boilperate plugin"
          DomainPath = "localization"
          Name = "WP CleanFix"
          Network = false
          PluginURI = "http://undolog.com"
          TextDomain = "wp-cleanfix"
          Title = "WP CleanFix"
          Version = "1.0.0"
        ...
        // Load text domain
        load_plugin_textdomain("wp-cleanfix", false, trailingslashit(basename($this->basePath)) . $this->DomainPath);
     ...
     *
     * @return \WPCleanFix\WPBones\View\View
     */
                    ...
                    if (isset($subMenu[ 'route' ][ 'load' ])) {
                        ...
                        add_action("load-{$subMenuHook}",
                            function () use ($controller, $method) {
                                $className = "WPCleanFix\\Http\\Controllers\\{$controller}";
                                $instance  = new $className;
